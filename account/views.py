...
def connexion(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request, 'Veuillez les caracteres valides dans le formulaire')
    else:
    ...
    if not request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                login(request, user)
                messages.success(
                    request, 'votre compte a été cree avec succes ')
                return redirect('articles:article_view')
        else:
...
def confirme(request, email):
    ...
    if request.method == 'POST':
        ...
        if password1 != password2:
            ...
            messages.error(
                request, 'Le mot de passe doit contenir au moins un chiffre')
        elif not any(char.isalpha() for char in password2):
            messages.error(
                request, 'Le mot de passe doit contenir au moins une lettre')
        else:
